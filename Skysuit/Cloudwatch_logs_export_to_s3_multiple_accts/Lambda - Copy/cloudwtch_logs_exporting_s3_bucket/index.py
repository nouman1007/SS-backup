...
def set_global_vars( ):
    ...
    try:
        ...
        global_vars["log_dest_bkt"]             = os.environ.get("Dest_Bucket_Name")           #dest_bkt
...
async def export_cw_logs_to_s3(global_vars, log_group_name, retention_days, bucket_name, obj_prefix = None):
    ...
    if '/' in log_group_name:
        d_prefix = str( log_group_name.replace("/","-")[1:] ) + "/" + str( gen_ymd(n_day, '/') )
    else:
        d_prefix = str( log_group_name.replace("/","-") ) + "/" + str( gen_ymd(n_day, '/') )
