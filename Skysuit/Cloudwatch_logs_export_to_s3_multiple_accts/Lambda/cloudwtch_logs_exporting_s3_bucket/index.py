...
def handler(event, context):
    ...
    print(global_vars.get('sns_topic_name'))
...
def set_global_vars( ):
    ...
    try:
        global_vars["sns_topic_name"]           = os.environ.get("topic_name") #'cloud-watch-export-test-v'
        global_vars["owner_email"]              = os.environ.get("Email")     #'nshafiq@enquizit.com' #                # owner_email
        global_vars["log_dest_bkt"]             = os.environ.get("Dest_Bucket_Name") #'eq-v2-522955560990-us-east-1'   # dest_bkt
...
def create_sns_topic(topic_name):
    ...
    logger.info('In Create sns topic')
...
def subscription (sns_topic_arn, email_address):
    ...
    logger.info('In email subscription.')
...
def publish(TopicArn, bucket):
    ...
    logger.info('In Published the Message.')

    client = boto3.client('sns')    
    response_publish = client.publish(
        TopicArn = TopicArn, 
        Message='Cloud watch LogGroups, their retention period is 24 hours remaining, has been sent This Bucket : '+ bucket,
        Subject='CloudWatch Logs Export Notification'
    )
