...
class Const():
    ...
    KEY_LOG_GROUP_NAME      = "LogGroup_name"
    KEY_RETENTION_DAYS      = "RetentionDays"
    KEY_DESTINATION_BUCKET  = "Dest_Bucket_Name"
...
def create(event, context):
    ...
    # Variables 
    reten_days = (event[Const.KEY_RESOURCE_PROPERTIES][Const.KEY_RETENTION_DAYS])
    cw_logs_name = (event[Const.KEY_RESOURCE_PROPERTIES][Const.KEY_LOG_GROUP_NAME])
    dest_bkt = (event[Const.KEY_RESOURCE_PROPERTIES][Const.KEY_DESTINATION_BUCKET])
    
    
    
    global_vars = set_global_vars(reten_days, cw_logs_name, dest_bkt )
...
def set_global_vars(reten_days, cw_logs_name, dest_bkt ):
    """
    ...
    try:
        global_vars["Owner"]                    = "EQ"
        global_vars["Environment"]              = "Dev"
        global_vars["aws_region"]               = "us-east-1"
        global_vars["tag_name"]                 = "serverless_cloudwatch_logs_exporter"
        global_vars["retention_days"]           = reten_days
        global_vars["cw_logs_to_export"]        = cw_logs_name
        global_vars["log_dest_bkt"]             = dest_bkt
