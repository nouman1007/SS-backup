...
for accountId in $ACCOUNTS_IN_ORGS; do
    ...
    test "$(jobs | wc -l)" -ge $PARALLEL_ACCOUNTS && wait || true
    {
        ...
        ./prowler/prowler -R "$ROLE" -A "$accountId" -g group4 -M json-asff -q -S -f us-east-1  >> jsonreports.json
        echo "Report stored locally at: prowler/output/ directory"
        
        # Upload Prowler Report to S3
        s3_account_session
        aws s3 cp prowler/output/ "$S3/reports/" --recursive --include "*.json-asff"
...
wait
cat jsonreports.json
#./prowler/output/scripts.sh
#aws s3 cp prowler/output/ "$S3/reports/" --recursive --include "*.json-asff"
mv scripts.sh prowler/output/
